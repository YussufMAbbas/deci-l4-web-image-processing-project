...
resizeImage.post("/", async (req, res) => {
    ...
    const imageOutputFolderPath = path.resolve(__dirname, "../../../images/resized");
    const imageOutputPath = path.resolve(imageOutputFolderPath, imageName + `-${w}x${h}.jpg`);
    if (!imageName || !w || !h) {
    ...
    if (fs.existsSync(path.resolve(imageOutputPath))) {
        ...
        res.send(fs.readFileSync(path.resolve(__dirname, "../../../images/resized", imageName + `-${w}x${h}.jpg`)));
